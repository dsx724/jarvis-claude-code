...
- **Desktop applications**: When opening desktop applications (e.g., Chrome), use the application's remote control flags to enable control over the application. For Chrome, use `google-chrome` with the remote debugging port flag (e.g., `--remote-debugging-port=9222`) so you can control the browser.
