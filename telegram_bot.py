...
def start_telegram_bot(token, allowed_user_ids, voice_replies, whisper_model,
    ...
    def _run():
        # Suppress the library's verbose traceback logging for network errors.
        ...
        while True:
            ...
            try:
                ...
                started = True
                if attempt > 0:
                    log.info("Telegram bot connected after %d retries", attempt)
                loop.run_forever()
                break  # clean exit from run_forever
            except Exception as e:
                attempt += 1
                log.warning("Telegram bot error (attempt %d): %s", attempt, e)
